...
from models import Game, LotteryTicket, Player, LotteryNumber
...
def parseTicketNumbers(numbers_in_string, ticketId):
	...
	for line in numbers_in_string.split('\n'):
		...
		number = LotteryNumber(ticket=ticket, 
			number1=int(linecontent[0]),
			number2=int(linecontent[1]),
			number3=int(linecontent[2]),
			number4=int(linecontent[3]),
			number5=int(linecontent[4]),
			ball=int(linecontent[5]))
